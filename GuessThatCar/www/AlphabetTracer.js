...
var CorrectSound = new Audio("./Resources/Sounds/68662__mikaelfernstrom__carreving.mp3");
var WrongSound = new Audio("./Resources/Sounds/131385__musicmasta1__carstartskidcrash.mp3");

var Progress = null;
var LastX = 0;
var LastY = 0;
var TotalDistance = 0;
var InitialDistance = 2500;
...
function GetLevel()
{
  ...
  var retVal = Math.floor(score / 100);

  return retVal > ImageFiles.length ? Math.floor(Math.random() * ImageFiles.length) : retVal;
}
...
function PopulateChoices()
{
  ...
  {
    var num = Math.floor(Math.random()* ImageFiles[Level].length);
    var name = ImageFiles[Level][num].name;
  ...
  var answer = ImageFiles[Level][CurrentImageIndex].name;
...
function ProcessChoice(ans)
{
  ...
  var correct = (ans == ImageFiles[Level][CurrentImageIndex].name);
  if (correct)
  {
    if (ShouldPlaySound)
    {
...
{
  LoadImageByName(ImageFiles[Level][index].src);
...
{
  CurrentImageIndex++;
  if (CurrentImageIndex >= ImageFiles[Level].length)
  {
...
function PreviousCanvas()
{
  ...
  {
    // cycle around to end
    CurrentImageIndex = ImageFiles[Level].length - 1;
  }
  LoadCanvas(CurrentImageIndex);
}
...
function RandomCanvas()
{
  ...
  while (true)
  {
    CurrentImageIndex = Math.floor(Math.random() * ImageFiles[Level].length);
